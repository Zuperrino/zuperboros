...
[![Version 6.46.2](https://img.shields.io/badge/version-6.46.2-green.svg)](VERSION)
...
|---------|------|-------------|
| 6.46.2 | 2026-06-25 | **fix: pre-flight environment checks in startup verification.** Three new boot-time checks surface evolution-cycle blockers before they cost mid-task rounds: `check_runtime_mode` reports the active runtime mode (informational), `check_git_identity` warns when `user.name`/`user.email` are missing (preventing the `Author identity unknown` catch-22 from cycle #1), and `check_python_path` uses `shutil.which` to detect `python3`/`python` availability. All three log to the `startup_verification` event in `events.jsonl`. Backlog item: ibl-6f27eca37ea2. |
| 6.46.1 | 2026-06-25 | **fix: allow read-only `git remote` subcommands through the shell-git policy.** `git remote`, `git remote -v`, `git remote get-url <name>`, and `git remote show` (bare) are now classified as read-only and allowed through `run_command`, matching the existing `branch`/`tag` readonly classifiers. `git remote show <name>` is network-gated (contacts the remote). Mutating forms (`add`, `remove`, `rm`, `rename`, `set-url`, `set-head`, `prune`, `update`) remain blocked. Removes a friction point hit twice in real tasks where `git remote -v` was blocked and required a `.git/config` file read fallback. |
